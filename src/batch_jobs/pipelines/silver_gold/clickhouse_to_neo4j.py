...
TRANSFORM_MAP = {
    "nodes": {  # Key is table name in clickhouse
        "movie": {
            ...
            "select_cols": ["movie_id", "original_title", "overview", "popularity", "release_date", "tagline",
                            "vote_average", "vote_count", "batch_version"],
        },
        "person": {
            "label": "Person", "keys": ["person_id"],
            "select_cols": ["person_id", "name", "gender", "also_known_as", "biography", "birthday", "deathday",
                            "place_of_birth", "known_for_department", "known_for_department", "batch_version"],
        },
        "tv_series": {
            "label": "TV_Series", "keys": ["tv_series_id"], "table_get_batch_version": "tv_series",
            "select_cols": ["tv_series_id", "overview", "popularity", "first_air_date", "tagline", "vote_average",
                            "vote_count", "status", "number_of_seasons", "batch_version"],
        },
    },
...
def process_nodes(
    ...
    """
    Process nodes data from ClickHouse
    """
...
def process_relationships(
    ...
    """
    Process relationships data from ClickHouse
    """
